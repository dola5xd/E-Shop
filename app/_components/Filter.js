...
import { ArrowDropDown, FilterAltRounded } from "@mui/icons-material";
...
function Filter() {
  ...
  return (
    <div className="px-5 pt-10 xl:w-1/3 h-full">
      <ul className="lg:flex flex-col gap-10 border rounded-2xl py-10 px-10 border-slate-500 w-full hidden">
      ...
      <span
        ...
        className="flex items-center flex-col-reverse gap-1 font-bold text-xl lg:hidden"
      >
        <h3>Filters</h3>
        <FilterAltRounded />
      </span>
      {mobileMenu && (
        <motion.ul
          className="flex flex-col gap-10 py-10 px-10 w-full lg:hidden bg-primary-darkWhite absolute z-40 top-0 left-0"
          variants={{
            open: { opacity: 1, display: "flex" },
            init: { opacity: 0, display: "hidden" },
          }}
          initial="init"
          animate={mobileMenu && "open"}
          transition={{
            duration: 0.7,
            staggerChildren: 1,
            delayChildren: 0.7,
